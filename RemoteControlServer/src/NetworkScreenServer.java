		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (e.getKeyCode() == 0) {
					...
					u32.keybd_event((byte) 0x15, (byte) 00, (byte) 0x0002, 0);// 누름
																				// 해제
				}
			}
		});
			...
			while (isRunning) {
				try {
					...
					if (isCompress) {
						...
						//byte[] compressImageByte = compress(imageByte);// 6MB->480KB정도						
			...
			try {
				...
				while (true) {
					...
					if (keyboardState == KEY_PRESSED) {// KEYBOARD PRESSED
						...
						// System.out.println(keyCode + "눌림");
						u32.keybd_event((byte) keyCode, (byte) 0, 0, 0);// 누름ffDDDddSS
						// robot.keyPress(keyCode);
					} else if (keyboardState == KEY_RELEASED) {
						int keyCode = dataInputStream.readInt();
						// System.out.println(keyCode + "때짐");
						u32.keybd_event((byte) keyCode, (byte) 00, (byte) 0x0002, 0);// 땜
